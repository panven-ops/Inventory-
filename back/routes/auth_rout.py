...
async def register(request: Request,user: UserCreate, _: None = Depends(register_limiter),db: Session = Depends(get_db)):
    register_user(db, user)

    return {"message": "User Created"}

@router.post("/login")
async def login(request: Request, user: UserLogin, _: None = Depends(login_limiter),db: Session = Depends(get_db)):
